    ...
    <nav aria-label="breadcrumb" class="breadcrumb-nav mb-2">
        <div class="container">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a>{{ $category_name }}</a></li>
                  ...
                  {{--      <ul class="pagination">
                            ...
                            <li class="page-item">
                                <a class="page-link page-link-next" href="#" aria-label="Next">
                                    Aprés <span aria-hidden="true"><i class="icon-long-arrow-right"></i></span>
                                </a>
                            </li>
                        </ul>--}}
                    </nav>
                                       ...
                                       @foreach ($categories as $category)
                                            <ul class="filter-item ">
                                                <li class="custom-control {{ count($category->subCategories) > 0 ? 'has-child-cate':''}}">
                                                    ...
                                                    <a data-toggle="collapse" href="#souswidget-{{ $category->id }}" role="button" aria-expanded="true" aria-controls="souswidget-{{ $category->id }}"><i class="icon icon-plus" ></i></a>
                                                     @if(count($category->subcategories)>0)

                                                            @foreach ($category->subCategories as $scategory )
                                                                <ul class="sub-cate" id="souswidget-{{ $scategory->category_id }}">
                                                                <li class="category-item {{ count($scategory->scategories) > 0 ? 'has-child-cate':''}}"" >
                                                                    <a href="{{ route('product.category',['category_slug'=>$category->slug,'scategory_slug'=>$scategory->slug]) }}" class="cat-link" >

                                                                        {{ $scategory->name }}
                                                                    </a>
                                                                        @if(count($scategory->scategories)>0)

                                                                            @foreach ($scategory->scategories as $sscategory )
                                                                                <ul class="sub-cate" ">

                                                                                    <a href="{{ route('product.category',['category_slug'=>$category->slug,'scategory_slug'=>$scategory->slug,'sscategory_slug'=>$sscategory->slug]) }}" class="scat-link" >
